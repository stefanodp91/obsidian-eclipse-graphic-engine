...
- `samples/endless-platformer-capacitor` wraps that same game in committed Android and iOS reference
  hosts, exercises Capacitor plugin discovery, and provides reproducible device deployment
  launchers;
...
- The native sample keeps platform generation, Android toolchain selection, target discovery,
  synchronization and device deployment reproducible without making Capacitor a dependency of the
  browser sample.
